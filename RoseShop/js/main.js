$(function () {
    ...
    var input = $(this).parent().prev();  // Lấy input số lượng
    var max = parseInt(input.attr('max'));  // Lấy giá trị max từ input
    var val = parseInt(input.val()) + 1;  // Tăng số lượng
    
    if (val <= max) {
        input.val(val);  // Cập nhật giá trị mới nếu chưa vượt max
    } else {
        $(".error-msg").text("Không thể thêm quá " + max + " sản phẩm!");
    }
    });
